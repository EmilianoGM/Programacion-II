...
namespace SP
{
    ...
    public partial class SegundoParcial: Form
    {
        ...
        private void btnPunto5_Click(object sender, EventArgs e)
        {
            ...
            this.c_bananas.EventoPrecio += new Cajon<Banana>.MiDelegado(manejadora.ManejadorPrecio);
        ...
        private static string ObtenerListadoFrutas()
        {
            ...
            try
            {
                ...
                comando.CommandText = "SELECT id,nombre,peso,precio FROM [sp_lab_II].[dbo].[frutas]";
                conexion.Open();
                SqlDataReader sqlDataReader = comando.ExecuteReader();
                while (sqlDataReader.Read())
                {
                    retorno += sqlDataReader["id"].ToString() + " - " + sqlDataReader["nombre"].ToString() + " - " + sqlDataReader["peso"].ToString() + " - " + sqlDataReader["precio"].ToString() + "\n";
                }
            } catch (Exception e)
            {
